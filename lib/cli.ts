...
program.command('wallet-init')
    ...
    .option('--filepath <string>', 'Filepath to use for wallet storage', '.')
    .option('--filename <string>', 'Filename to use for wallet storage', 'wallet.json')
    .action(async (options) => {
        try {
            const result = await Atomicals.walletInit(options.phrase, options.path, options.filepath, options.filename, options.n ? parseInt(options.n, 10) : undefined);
...
program.command('mint-dft')
    ...
    .argument('<ticker>', 'string')
    .option('--rbf', 'Whether to enable RBF for transactions.')
    .option('--toaddr <string>', 'Assign claimed tokens into this address')
    .option('--url <string>', 'URL to the minting file')
    ...
    .action(async (ticker, options) => {
        try {
            ...
            const result: any = await atomicals.mintDftInteractive({
                ...
                satsbyte: parseInt(options.satsbyte),
                disableMiningChalk: options.disablechalk,
            }, options.toaddr, ticker, options.funding);
            handleResultLogging(result, true);
        } catch (error) {
            console.log(error);
        }
