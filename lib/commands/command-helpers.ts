...
export var NETWORK = process.env.NETWORK === 'testnet' ? networks.testnet : process.env.NETWORK == "regtest" ? networks.regtest : networks.bitcoin;

export function changeNetwork(network: string) {
    NETWORK = network === 'testnet' ? networks.testnet : network == "regtest" ? networks.regtest : networks.bitcoin;
}
