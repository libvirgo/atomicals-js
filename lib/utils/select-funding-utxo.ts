 ...
 * @param address
 * @param amount
 * @param suppressDepositAddressInfo
 * @param seconds
 * @param network
 * @returns
 */
export const getFundingUtxo = async (electrumxApi, address: string, amount: number, suppressDepositAddressInfo = false, seconds = 5, network = NETWORK) => {
    ...
    // console.log(`...`)
    // console.log(`...`)
    const fundingUtxo = await electrumxApi.waitUntilUTXO(address, amount, seconds ? 5 : seconds, false, network);
